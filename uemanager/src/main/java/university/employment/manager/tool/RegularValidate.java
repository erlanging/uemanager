...
/**
 * 此工具类主要进行邮箱，电话号码，手机，身份证号,IP地址，
 * Integer格式，正整数、年龄，邮编，过滤特殊字符，检查是否含有特殊字符
 * @author liang
 *
 */
public class RegularValidate {
     ...
     * 
     */ 
    public static final String PHONE = "(^(0[0-9]{2,3}[-_－—]?)?([2-9][0-9]{6,7})+([-_－—]?[0-9]{1,4})?$)" ;  
    /** 
     * 手机号码正则表达式=^(13[0-9]|15[0|3|6|7|8|9]|18[0,5-9])\d{8}$ 
     */ 
    public static final String MOBILE ="^((13[0-9])|(14[5,7,9])|(15[^4,\\D])|(17[0,1,3,5-8])|(18[0-9]))\\d{8}$";  
     ...
     * URL正则表达式 
      * 匹配 http https
     */ 
    public static final String URL2 = "^(http|https|)?(://)?(\\w+(-\\w+)*)(\\.(\\w+(-\\w+)*))*((:\\d+)?)(/(\\w+(-\\w+)*))*(\\.?(\\w)*)(\\?)?" +  
    "(((\\w*%)*(\\w*\\?)*(\\w*:)*(\\w*\\+)*(\\w*\\.)*(\\w*&)*(\\w*-)*(\\w*=)*(\\w*%)*(\\w*\\?)*" +  
    "(\\w*:)*(\\w*\\+)*(\\w*\\.)*" +  
    "(\\w*&)*(\\w*-)*(\\w*=)*)*(\\w*)*)$";   
    ...
    /** 
     * 判断是否为Url 符合返回ture http|www|ftp start
     * @param str 
     * @return boolean 
     */ 
    ...
    /** 
     * 判断是否为Url 符合返回http |https start
     * @param str 
     * @return boolean 
     */ 
    public static  boolean isUrl(String str) {  
        return Regular(str,URL2);  
    }
    ...
    public static boolean containsSpecialCharacter(String str){
    	for(int i=0;i<str.length();i++){
    		if(Regular(str.substring(i, i+1), STR_SPECIAL)){
    			return true;
    		}
    	}
    	return false;
    ...
    public static  boolean isScientific(String str){  
        if(StrisNull(str))  
            return false;   
        return Regular(str,RegularValidate.SCIENTIFIC_A);  
    }  
}
